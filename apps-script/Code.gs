...
var SHEETS={
  ...
  news:'News',demands:'Demands',documents:'Documents',meetings:'Meetings',notifications:'Notifications',polls:'Polls',participation:'Participation',sessions:'AuthSessions'
};
var HEADERS={
...
function saveMember_(d){var lock=LockService.getScriptLock();lock.waitLock(5000);try{var ss=ensureSheets(),sh=ss.getSheetByName('Responses'),phone=cleanPhone_(d.phone);if(!validPhone_(phone))throw new Error('Valid 10-digit mobile required');if(!validPin_(d.pin))throw new Error('PIN must be 4 digits');var r=findMemberRow_(sh,phone);if(r>=0)throw new Error('Mobile already registered. Please login.');var name=requireLength_(d.name,100,'Name');memberId=MEMBER_ID_PREFIX+String(Math.max(1,sh.getLastRow())).padStart(4,'0');sh.appendRow([new Date(),memberId,name,phone,limit_(d.category,30),limit_(d.vendor,100),limit_(d.division,80),limit_(d.joinYear,10),limit_(d.experience,30),limit_(d.salary,30),limit_(d.demands,1000),hashPin_(d.pin)]);var session=issueSession_(phone);return jsonOut_({success:true,message:'Registration successful',member:{memberId:memberId,name:name,phone:phone,division:d.division,category:d.category},sessionToken:session.token,expiresAt:session.expiresAt})}finally{lock.releaseLock()}}
...
function adminSaveNews_(d){requireAdmin_(d);if(!validUrl_(d.url)||!validUrl_(d.imageUrl))throw new Error('Only http/https URLs are allowed');var id=upsert_('News',d.id,[d.id||'',new Date(),d.date||new Date(),limit_(d.category,60)||'Employee News',limit_(d.headline,200),limit_(d.summary,2000),limit_(d.source,200),d.url||'',d.imageUrl||'',truthy_(d.published)]);if(truthy_(d.notify))sendPushNotification_('नई News',d.headline||'नई खबर उपलब्ध है',d.url||'');return jsonOut_({success:true,message:'News saved',id:id})}
