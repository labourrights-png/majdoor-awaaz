...
function memberSheet_(ss){
  ...
  if(legacy&&legacy.getLastRow()>1&&legacyMemberHeader_(legacy))return legacy;
  return sh;
}
...
function migrateLegacyMembers_(ss){
  var target=ss.getSheetByName('Responses'),legacy=ss.getSheetByName('Sheet1');
  ...
  rows.forEach(function(r){
    ...
    if(!validPhone_(phone))return;
    out.push([
      ix.ts>=0&&r[ix.ts]?r[ix.ts]:new Date(),
      ix.id>=0&&r[ix.id]?r[ix.id]:MEMBER_ID_PREFIX+Utilities.getUuid().slice(0,8).toUpperCase(),
      ix.name>=0?r[ix.name]:'',
      phone,
      ix.category>=0?r[ix.category]:'',
      ix.vendor>=0?r[ix.vendor]:'',
      ix.division>=0?r[ix.division]:'',
      ix.joinYear>=0?r[ix.joinYear]:'',
      ix.experience>=0?r[ix.experience]:'',
      ix.salary>=0?r[ix.salary]:'',
      ix.demands>=0?r[ix.demands]:'',
      ix.pin>=0?r[ix.pin]:''
    ]);
  });
  if(out.length)target.getRange(2,1,out.length,RESP_HEADERS.length).setValues(out);
}
...
function readMember_(phone,pin){var sh=memberSheet_(),r=findMemberRow_(sh,phone);if(r<0)return null;var row=sh.getRange(r,1,1,RESP_HEADERS.length).getValues()[0];if(pin!==undefined&&hashPin_(pin)!==String(row[11]||''))return null;return memberFromRow_(row)}
...
function adminResetPin_(d){requireAdmin_(d);var phone=cleanPhone_(d.phone);if(!validPin_(d.newPin))throw new Error('PIN must be 4 digits');var sh=memberSheet_(),r=findMemberRow_(sh,phone);if(r<0)throw new Error('Member not found');sh.getRange(r,12).setValue(hashPin_(d.newPin));return jsonOut_({success:true,message:'PIN reset'})}
function adminUpdateMember_(d){requireAdmin_(d);var sh=memberSheet_(),r=findMemberRow_(sh,d.phone);if(r<0)throw new Error('Member not found');var old=sh.getRange(r,1,1,RESP_HEADERS.length).getValues()[0];['name','category','vendor','division','joinYear','experience','salary','demands'].forEach(function(k){var col={name:3,category:5,vendor:6,division:7,joinYear:8,experience:9,salary:10,demands:11}[k];if(d[k]!==undefined)old[col-1]=d[k]});sh.getRange(r,1,1,RESP_HEADERS.length).setValues([old]);return jsonOut_({success:true,message:'Member updated'})}
function adminDeleteMember_(d){requireAdmin_(d);var sh=memberSheet_(),r=findMemberRow_(sh,d.phone);if(r<0)throw new Error('Member not found');var ss=ensureSheets(),del=ss.getSheetByName('DeletedMembers')||ss.insertSheet('DeletedMembers');if(del.getLastRow()===0)del.appendRow(RESP_HEADERS.concat(['DeletedAt']));del.appendRow(sh.getRange(r,1,1,RESP_HEADERS.length).getValues()[0].concat([new Date()]));sh.deleteRow(r);return jsonOut_({success:true,message:'Member archived'})}
...
function publicNews_(){return sortDesc_(published_('News'),'Date').slice(0,80).map(function(x){return {id:x.ID,createdAt:x.CreatedAt,date:x.Date,category:x.Category,headline:x.Headline,summary:x.AiSummary||x.Summary,aiSummary:x.AiSummary||'',source:x.Source,url:x.Url,imageUrl:x.ImageUrl,videoUrl:x.VideoUrl||'',tags:x.Tags||'',provider:x.Provider||'News'}})}
function publicShorts_(){var employee=rows_('VideoSubmissions').filter(function(x){return String(x.Status||'').toLowerCase()==='published'&&x.VideoUrl}).map(function(x){return {id:'employee-'+x.ID,createdAt:x.CreatedAt,date:x.PublishedAt||x.CreatedAt,category:'कर्मचारी वीडियो',headline:x.Title,summary:x.Caption||'कर्मचारी द्वारा साझा किया गया वीडियो',source:x.Name||'कर्मचारी',url:x.VideoUrl,videoUrl:x.VideoUrl,imageUrl:'',tags:'कर्मचारी वीडियो',provider:'Employee Upload'}});var news=publicNews_().filter(function(x){return !!x.videoUrl});return employee.concat(news).sort(function(a,b){return new Date(b.date||b.createdAt)-new Date(a.date||a.createdAt)}).slice(0,60)}
