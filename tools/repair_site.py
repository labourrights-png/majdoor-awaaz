...
api_block = """async function api(body){let r=await fetch(SCRIPT_URL,{redirect:'follow',method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(body),cache:'no-store'});let t=await r.text();return JSON.parse(t.replace(/^\\uFEFF/,'').trim())}
